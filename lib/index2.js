...
const https = require('https'); // expressjs configuration


const app = express();
const port = 3000;
app.set('view engine', 'pug'); // Middleware function for async requests within expressjs
...
app.use(express.static('public')); // Set the expressjs router with an endpoint for getting FLO data
...
const getFloDataHttp = async () => {}; // js-oip API calls recent tZERO DLR records


let api = new _jsOip.DaemonApi("api.oip.io");
...
const getFloData = async () => {
  ...
  let _ref = await api.searchFloData(query, limit),
...
function formatTime(oipdata) {
  for (let tx of oipdata) {
    ...
    tx.tx.timeago = ta.ago(tx.tx.time * 1000); // Set tx.tx.time to human readable time
